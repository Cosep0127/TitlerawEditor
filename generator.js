...
function componentToObj(comp) {
  ...
  switch (comp.type) {
    ...
    case 'translate':
      ...
      if (comp.withType === 'object' && comp.withComponents && comp.withComponents.length > 0) {
        obj.with = { rawtext: comp.withComponents.map(componentToObj).filter(Boolean) };
      } else if (comp.with) {
...
export function generateCommand() {
  const rawtext = state.components.map(componentToObj).filter(Boolean);
